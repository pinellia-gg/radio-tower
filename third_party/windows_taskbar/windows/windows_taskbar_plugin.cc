...
class WindowsTaskbarPlugin : public flutter::Plugin {
 ...
 private:
  ...
  void ApplyCachedThumbnailToolbar();
  ...
  std::unique_ptr<flutter::MethodChannel<flutter::EncodableValue>> channel_ =
      nullptr;
  std::optional<std::vector<ThumbnailToolbarButton>> thumbnail_toolbar_;
  std::optional<std::string> thumbnail_tooltip_;
};
...
void WindowsTaskbarPlugin::ApplyCachedThumbnailToolbar() {
  if (!taskbar_ready_ || windows_taskbar_ == nullptr) {
    return;
  }

  // Windows requires ThumbBarAddButtons again after Explorer has recreated
  // the taskbar button. Resetting here ensures the next call adds, rather
  // than updates, the thumbnail toolbar.
  windows_taskbar_->ResetThumbnailToolbarRegistration();
  if (thumbnail_toolbar_.has_value()) {
    windows_taskbar_->SetThumbnailToolbar(*thumbnail_toolbar_);
  }
  if (thumbnail_tooltip_.has_value()) {
    windows_taskbar_->SetThumbnailTooltip(*thumbnail_tooltip_);
  }
}
...
void WindowsTaskbarPlugin::HandleMethodCall(
  ...
  if (method_call.method_name().compare(kSetProgressMode) == 0) {
    ...
    thumbnail_toolbar_ = thumbnail_toolbar_buttons;
    if (!taskbar_ready_) {
      // Shell sends TaskbarButtonCreated only after the taskbar button exists.
      // Calling ThumbBarAddButtons before then permanently prevents retries.
      result->Success(flutter::EncodableValue(std::monostate{}));
      return;
    }
